...
describe('checkMatch', () => {
  test('Совпадение дает очки', () => {
    ...
    expect(showScorePopup).toHaveBeenCalled()
  })
  ...
  test('luckyPair дает бонус +5', () => {
    ...
    expect(showScorePopup).toHaveBeenCalledWith('+10+5')
  })
  ...
  test('Несовпадение с увиденными картами применяет пенальти', () => {
    ...
    expect(showScorePopup).toHaveBeenCalledWith(expect.stringContaining('-'))
    expect(set.setScore).toHaveBeenCalled()
  })
  ...
  test('Несовпадение без пенальти не показывает popup', () => {
    ...
    expect(showScorePopup).not.toHaveBeenCalled()
  })
